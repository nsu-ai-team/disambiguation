"""
This file is based on https://github.com/ryankiros/skip-thoughts/blob/master/training/tools.py
...
def encode(model, X, use_norm=True, verbose=True, batch_size=128, use_eos=True):
    """
...
def load_googlenews_vectors(path, binary = True):
    """
    load the word2vec GoogleNews vectors
    """
    embed_map = gensim.models.KeyedVectors.load_word2vec_format(path, binary=True, unicode_errors='ignore')
    return embed_map
...
def train_regressor(options, embed_map, wordvecs, worddict):
    ...
    w2v_dim = embed_map.word_vec(list(embed_map.vocab.keys())[0]).shape[0]
    w2v = numpy.zeros((len(shared), w2v_dim), dtype='float32')
